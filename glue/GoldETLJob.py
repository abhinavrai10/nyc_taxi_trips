...
job = Job(glueContext)
# dyf = glueContext.create_dynamic_frame.from_catalog(database='nyc_taxi_db', table_name='silver_yellow_taxi')
# dyf.printSchema()
# df_silver = dyf.toDF()
# df_silver.show(5)
...
try:
    ...
    max_date = fact_trips_existing.select(F.max(col("pickup_date"))).collect()[0][0]
except AnalysisException:
...
airport_flags = df_silver.groupBy("pickup_location_id").agg(
    ...
    df_silver.groupBy("dropoff_location_id").agg(
        (max("airport_fee") > 0).alias("is_airport_pickup")  # Use consistent column name
    ).withColumnRenamed("dropoff_location_id", "location_id")
).groupBy("location_id").agg(
    max("is_airport_pickup").alias("is_airport")  # Aggregate over consistent column
)
...
try:
    ...
    max_payment_type_id = get_max_id("s3://lakehouse-nyc-taxi/gold/dim_payment_type/", "payment_type")
except AnalysisException:
...
# Select and rename columns for fact_trips
